...
// */
let linewidth = 5;
const canvasOffsetX = draw.offsetLeft;
const canvasOffsetY = draw.offsetTop;

draw.width = window.innerHeight - canvasOffsetX;
draw.height = window.innerHeight - canvasOffsetY; 

// for(let i = 0; i < 1080; ++i){
//     const newDiv = document.createCanvas;
...
canvas.addEventListener('click' , e => {
    if (e.target.id === 'clear') {
        button.clearRect(0, 0, draw.width, draw.height);
        }
...
canvas.addEventListener('change', e=>{
    if(e.target.id === 'stroke') {
        button.strokeStyle = e.target.value;
    }
